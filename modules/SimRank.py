...
class BipartiteSimRankPP(BipartiteSimRank):
    def __init__(self):
        super(BipartiteSimRankPP, self).__init__()
        self.Evidence_N1 = pd.DataFrame()
        self.Evidence_N2 = pd.DataFrame()
        self.Weight_N1 = pd.DataFrame()
        self.Weight_N2 = pd.DataFrame()
        
    def _cal_Evidence(self, G, obj, verbose):
        if verbose:
    ...
    def _cal_Weight(self, G, obj, verbose):
        if verbose:
    ...
    def fit(self, data, C1 = 0.8, C2 = 0.8, weighted = False, node_group1_column = 'user', node_group2_column = 'item', weight_column = 'weight', iterations = 100, eps = 1e-4, verbose = True):
        ...
        self.Weight_N1 = self._cal_Weight(self.Graph_N1_N2, 'NodeGroup1', verbose)
        self.Weight_N2 = self._cal_Weight(self.Graph_N2_N1, 'NodeGroup2', verbose)
        # Cal E for user and item
        self.Evidence_N1 = self._cal_Evidence(self.Graph_N1_N2, 'NodeGroup1', verbose)
        self.Evidence_N2 = self._cal_Evidence(self.Graph_N2_N1, 'NodeGroup2', verbose)
